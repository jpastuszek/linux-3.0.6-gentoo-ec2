...
/*
 * Trackstick E7 reports for models affected by trackstick button
 * reporting quirk
 */
static const unsigned char trackstick_quirk_sigs[][3] = {
	{ 0x42, 0x02, 0x14 },
	{ 0x32, 0x01, 0x28 },
};
...
static int alps_hw_init_v3(struct psmouse *psmouse)
{
	...
	if (reg_val & 0x80) {
		...
		if (ps2_command(ps2dev, NULL, PSMOUSE_CMD_SETSCALE21) ||
			...
			/*
			 * Most models report trackstick buttons in the
			 * touchpad packets, but some report them in the
			 * trackstick packets instead. It appears we can
			 * detect these using the trackstick E7 report.
			 */
			for (i = 0; i < ARRAY_SIZE(trackstick_quirk_sigs); i++) {
				if (!memcmp(param, trackstick_quirk_sigs[i],
					    sizeof(trackstick_quirk_sigs[i]))) {
					pr_debug("alps.c: enabling trackstick button quirk\n");
